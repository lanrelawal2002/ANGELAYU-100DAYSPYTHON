# my fourth version of the higher lower project
# I used four(4) functions
# Complies with the real Higher Lower game in changing options(characters)
import random
...
def reverse_second_dictionary(first_dictionary, second_dictionary):
    repeat_check = True
    redundancy_counter = 0

    while repeat_check:
        first_dictionary_index = data.index(first_dictionary)
        new_index = random.randint(0, end_range)
        second_dictionary_index = data.index(second_dictionary)
        if new_index == first_dictionary_index or new_index == second_dictionary_index:
...
def compute(first_dictionary, second_dictionary):
    ...
    while correct_count:
        ...
        if choice == "A" and first_dictionary['follower_count'] > second_dictionary['follower_count']:
            ...
            switched_second_dictionary = reverse_second_dictionary(first_dictionary, second_dictionary)
